...
public class MetadataParser {
   ...
   * 
   * @return A number indicating the orientation of the image. 0-axial, 1-coronal, 2-sagittal.
   */
   ...
   * 
   * @return An array of three positive float number in the order of x_dimension, y_dimension and
   *         z_dimension.
   */
   ...
   * Gets a voxel's dimention or resolusion.
   * 
   * @return An array of three signed float number in the order of x_dimension, y_dimension and
   *         z_dimension.
   */
   ...
   * 
   * @param maxArray An array containing the maximum intensity values of all the imaging slices.
   * 
   * @return A renormalization array, or null if the data does not need to be renormalized..
   */
  ...
  private double _getAge(Node node, String format) {
    double age = 40;
    if (format.equals(FORMAT_TYPES[1]))
      age = _getMincAge(node);
    return age;
  }
  ...
  private float[] _getAnalyzePixDims(Node node) {
    ...
    while (node != null) {
       ...
       * System.out.println("Node Name:  "+node.getNodeName()); map = node.getAttributes();
       * attribute = map.getNamedItem("orient"); orient = (new
       * Integer(attribute.getNodeValue())).intValue(); } else
       */
      if (node.getNodeName().equals("IMAGE_DIMENSION")) {
        map = node.getAttributes();
        for (int k = 0; k < 3; k++) {
  ...
  private int _getBitsPP(Node node, String format) {
    ...
    if (format.equals(FORMAT_TYPES[0]))
      bitsPP = _getAnalyzeBitsPP(node);
    else if (format.equals(FORMAT_TYPES[1]))
      bitsPP = _getMincBitsPP(node);
    return bitsPP;
  }
  ...
  /**
   * Parses Dicom metadata for the images' resolusions, orientation and the number of bits per
   * pixel.
   * 
   * @param node Associated with the queried file.
   * 
   * @return A array of float values containing images' resolusions, orientation and the number of
   *         bits per pixel.
   */
   ...
   * Parses Dicom metadata for the image's orientation.
   * 
   * @return The value of the image's orientation. 0-axial, 1-coronal, 2-sagittal.
   */
  ...
  /**
   * Gets the max pixel values, each corresponding to the maximum pixel value of one slice.
   * 
   * @param node Node of the file header.
   * @param numOfImages Number of images.
   * 
   * @return An array of max pixel values, or null if header does not carry the information.
   */
  ...
  /**
   * Gets the minimum pixel values, each corresponding to the minimum pixel value of one slice.
   * 
   * @param node Node of the file header.
   * @param numOfImages number of images.
   * @return array of min pixel values, or null if header does not carry the information.
   */
  ...
  /**
   * Gets Maximum or Minimum pixel values, each corresponding to the maximum or minimum pixel value
   * of one slice.
   * 
   * @param node Node of the file header.
   * @param numOfImages Number of images.
   * @param nodeName Name of the node containing the max or min pixel values.
   * 
   * @return An array of max or min pixel values, or null if header does not carry the information.
   */
  private double[] _getMaxOrMinPixelValues(Node node, int numOfImages, String nodeName) {
    double[] max = null;
    NamedNodeMap map;
    Node attribute;
    while (node != null) {
                ...
                for (int m = 0; m < listUnderDATA.getLength(); m++) {
                  ...
                  if (nodeForARRAY.getNodeName().equals("ARRAY")) {
                    ...
                    for (int k = 0; k < max.length; k++) {
                      ...
                      attribute = map.getNamedItem("value");
                      max[k] = (new Double(attribute.getNodeValue())).doubleValue();
                    }
                  }
                }
              }
   ...
   * 
   * @param node Node of the file header.
   * @return A two-element double array: the first element is the minimum intensity value of the
   *         data and the second element is the maximum intensity value of the data.
   */
  ...
  private int _getMincOrient(Node node) {
                ...
                for (int n = 0; n < listUnderATT.getLength(); n++) {
                  ...
                  if (nodeForDirCos.getNodeName().equals("direction_cosines")) {
                    ...
                    for (int m = 0; m < dims.length; m++) {
                      ...
                      attribute = map.getNamedItem("value");
                      dims[m] = (new Float(attribute.getNodeValue())).floatValue();
                    }
                  }
                }
              }
    ...
    if (dims != null) {
      if ((Math.abs(dims[0]) > Math.abs(dims[1])) && (Math.abs(dims[0]) > Math.abs(dims[2]))) {
        orient = 2;
      } else if ((Math.abs(dims[1]) > Math.abs(dims[0])) && (Math.abs(dims[1]) > Math.abs(dims[0]))) {
        orient = 1;
      } else
        orient = 0;
    } else {
      if (spaceName.equals("xspace"))
        orient = 2;
      else if (spaceName.equals("yspace"))
        orient = 1;
      else
        orient = 0;
    }
  ...
  private int _getOrient(Node node, String format) {
    ...
    if (format.equals(FORMAT_TYPES[0]))
      orient = _getAnalyzeOrient(node);

    // Check Minc file format and get the pixdims
    else if (format.equals(FORMAT_TYPES[1]))
      orient = _getMincOrient(node);
    ...
    else if (format.equals(FORMAT_TYPES[4]))
      orient = 1;
    else
      orient = 0;
    return orient;
  ...
  private float[] _getPixDims(Node node, String format) {
    ...
    // already in the order of dim_x, dim_y and dim_z
      ...
      return _getMincPixDims(node);

    else if (format.equals(FORMAT_TYPES[2]))
      return _getDicomPixDims(node);

    else if (format.equals(FORMAT_TYPES[3]))
      return _getGEPixDims(node);
  ...
  private float[] _getPixDims(Object[] pixDimsArray, String format, int orient) {
    ...
    if (format.equals(FORMAT_TYPES[2])) {
      ...
      if (orient == 0)
        pixDims[2] = pos1[2] - pos0[2];
      else if (orient == 1)
        pixDims[2] = pos1[1] - pos0[1];
      else
        pixDims[2] = pos1[0] - pos0[0];
  ...
  /**
   * Check if all the files have the same file format, orient, bitsPP and pixDims.
   * 
   * @param formatArray An array of String values. Each element is the format of an image file.
   * @param orientArray An array of int value. Each element is the orientation of an image file.
   * @param bitsPPArray An array of int values. Each element is the number of bits per pixel of an
   *        image file.
   * @param pixDimsArray An array ob Ojbects. Each element is a two element array indicating the
   *        pixel's dimensions in x and y axises.
   */
  private boolean _isSame(String[] formatArray, int[] orientArray, int[] bitsPPArray,
      Object[] pixDimsArray) {
    boolean isSame = true;
    float[] pixDims1;
    float[] pixDims2;
    for (int i = 0; i < (formatArray.length - 1); i++) {
      ...
      if (formatArray[i].equals(formatArray[i + 1]) && orientArray[i] == orientArray[i + 1]
          && bitsPPArray[i] == bitsPPArray[i + 1] && pixDims1[0] == pixDims2[0]
          && pixDims1[1] == pixDims2[1]) {
      } else {
        isSame = false;
        break;
      }
