...
public class DataVolume
{
   ...
   * @param rotate The number of rotation of the images provided by the given imageReader. The
   *        images would be rotated (number * 90) degrees clockwisely.
   ...
   */
  public DataVolume(ImageReader reader, int rotate, boolean isXFlipEnabled, boolean isYFlipEnabled)
  {

    if (reader == null)
    {
      IJ.error("Image Reader is null");
    }
    // Create a MetadataParser object
    _metadataParser = new MetadataParser(reader);
    BufferedImage[] images = _getBufferedImages(reader, rotate, isXFlipEnabled, isYFlipEnabled);
    ...
    {
      _volByte =
          _createByteDataVolume(images, _metadataParser.getOrient(),
              _metadataParser.getReNormValues());
      _histogram = _calculateHistogram(_volByte);
    } else
    {
      _volShort =
          _createShortDataVolume(images, _metadataParser.getOrient(),
              _metadataParser.getReNormValues());
   ...
   */
  public DataVolume(BufferedImage[] images, int orient, int bitsPP, String format)
  {
    // Create data volume
    if (bitsPP <= 8)
    {
  ...
  /**
   * Gets the intensity value below which lies 2% of the cumulative histagram.
   * 
   ...
   * Gets voxels' dims.
   * 
   * @return A float array contains three elements with each representing the dimension on X, Y and
   *         Z directions.
   */
   ...
   * 
   * @return An array of scale ratios in the order of the ratio in Y direction, the ratio in X
   *         direction and the ratio in Z direction.
   */
   ...
   * 
   * @param orient Orientation of the section to be queried. If orient is 0, it is axial; 1 is
   *        coronal; and 2 is sagittal.
   * 
   * @throws IllegalArgumentException If the orientation value is invalid.
   * 
   * @return An array of the scale ratios of the section in the given orientation. The order of the
   *         array elements is scale ratio in X direction and scale ratio in Y direction.
   */
  public float[] getScaleRatios(int orient)
  {
    if (orient >= 3)
    {
      throw new IllegalArgumentException("DataVolume: " + orient + " is not a valid orientation. "
          + "A valid orientation is 0, 1, " + "or 2 which indicates axial, "
   ...
   * 
   * @param orient Orientation of the sections to be queried. If orient is 0, it is axial; 1 is
   *        coronal; and 2 is sagittal.
   * 
   * @throws IllegalArgumentException If the orientation value is invalid.
   * 
  ...
  public int getValue(int x, int y, int z)
  {
    int temp;
    if (_volByte != null)
      temp = _volByte[z][x][y] & 0xff;
    else
      temp = _volShort[z][x][y] & 0xffff;
    return temp;
  }
  ...
  private byte[][][] _createByteDataVolume(BufferedImage[] images, int orient)
  {
    ...
    try
    {
      ...
      int tempInt;
      /**
       * Get the images orientation: 0 transverse unflipped 1 coronal unflipped 2 sagittal unflipped
   ...
   * @return A 3D array of BYTE numbers.
   */
  private byte[][][] _createByteDataVolume(BufferedImage[] images, int orient, double[] renorm)
  {
    if (renorm == null)
      return _createByteDataVolume(images, orient);
    byte[][][] vol = null;
    int width = 0;
    int height = 0;
    int depth = 0;
   ...
   */
  private short[][][] _createShortDataVolume(BufferedImage[] images, int orient)
  {

    IJ.log("CreateShortDataVolumn: " + orient);
    ...
    IJ.log("W/H/D = " + _width + "/" + _height + "/" + _depth);
   ...
   * @return A 3D array of SHORT numbers.
   */
  private short[][][] _createShortDataVolume(BufferedImage[] images, int orient, double[] renorm)
  {

    IJ.log("Create data volumn(short): " + orient);
    IJ.log("Renorm: " + renorm);

    if (renorm == null)
      return _createShortDataVolume(images, orient);
    short[][][] vol = null;
    int width = 0;
    int height = 0;
    int depth = 0;
   ...
   */
  private BufferedImage _flipImage(BufferedImage image, boolean xFlip, boolean yFlip)
  {
    if ((!xFlip) && (!yFlip))
      return image;
    if (image == null)
  ...
  private BufferedImage[] _getBufferedImages(ImageReader reader, int rotate,
      boolean isXFlipEnabled, boolean isYFlipEnabled)
  {
    ...
    try
    {
      ...
      for (int i = 0; i < bImgs.length; i++)
      {
        ...
        tempImage = _flipImage(tempImage, isXFlipEnabled, isYFlipEnabled);
        bImgs[i] = tempImage;
      }
  ...
  /**
   * Calculates the histogram of the data volume and compute the value of _intens2, under which
   * there are 2% pixel intensities.
   * 
   ...
   * @param rotateNumber The number of the rotation. The degrees of rotation will be (the number of
   *        rotation * 90).
   * 
